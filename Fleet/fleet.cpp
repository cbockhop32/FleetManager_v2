...
void
Fleet::AddCarToFleet(Car& newCar)
{
...
void
Fleet::RemoveCarFromFleet(std::string vin)
{
  if (!CarExists(vin)) {
    ...
    std::cout << "\nYou want to remove the following car: " << car.year << " "
              << car.make << " " << car.model << '\n';
...
Fleet::ListCarsInFleet()
{
  ...
  if (_fleet.size() == 0) {
    ...
    for (auto& [vin, car] : _fleet) {
      std::cout << count << " : " << car.getVIN() << " " << car.year << " "
                << car.make << " " << car.model << " " << car.mileage << " "
                << car.cityMPG << " " << car.highwayMPG << " "
                << car.getPurchasePrice() << " " << car.getCurrentValue()
                << '\n';
      count++;
    }
  }
...
void
Fleet::GetCarDetails(std::string vin)
{
  if (!CarExists(vin)) {
...
Car&
Fleet::GetCarByVIN(std::string vin)
{
...
Fleet::GetFleetStats()
{
  if (_fleet.size() == 0) {
    ...
    float totalReturn{ (1 - (float)totalCurrentVal / (float)totalPurchase) *
                       100 };

    std::cout.imbue(std::locale(""));

    std::cout << "\n___Fleet Statistics___\n";
    std::cout << "Total Number of Cars: " << _fleet.size() << "\n";
    std::cout << "Average Original Cost (Per Vehicle): $"
              << totalPurchase / _fleet.size() << "\n";
    std::cout << "Average Current Value (Per Vehicle): $"
              << totalCurrentVal / _fleet.size() << "\n";
    std::cout << "Average Mileage (Per Vehicle): "
              << totalMileage / _fleet.size() << "\n";
    std::cout << std::fixed << std::setprecision(2)
              << "Average Loss (Per Vehicle): " << totalReturn << "%" << '\n';

    std::cout << "Average Fuel Economy (City/Highway MPG): "
              << totalCityMPG / _fleet.size() << " / "
              << totalHighwayMPG / _fleet.size() << '\n';
  }
};

bool
Fleet::CarExists(std::string vin)
{
