...
Car::Car(std::string VIN, std::string year, std::string make, std::string model, std::string mileage, std::string citympg, std::string highwaympg, std::string price, std::string currVal) 
: year{year}, make{make}, model{model}, mileage{mileage}, cityMPG{citympg}, highwayMPG{highwaympg} {
    setVIN(VIN);
...
void Car::EditCarDetails(){
    ...
    std::vector<std::string> detailPrompts = {"Year", "Make", "Model", "Mileage", "City Miles Per Gallon", "Highway Miles Per Gallon", "Original Purchase Price", "Current Market Value"};
    int currIdx = 1;

    for(std::string &currDetail : detailPrompts) {
        std::cout << "Enter a value for " << currDetail << "(Current: " <<this->operator[](currIdx) <<"): ";
        std::string newVal;
